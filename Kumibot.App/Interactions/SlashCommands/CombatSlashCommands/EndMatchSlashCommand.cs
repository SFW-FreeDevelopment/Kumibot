...
public class EndMatchSlashCommand : InteractionBase
{
    ...
    public async Task EndMatch()
    {
        ...
        if (combatEvents.Count > 0)
        {
            ...
            var combatEventSelectList = SelectListHelper.GetSelectList(Constants.EndMatchCombatEventSelectListId, selectListOptions.ToDictionary(x => x.EventTitle, x=> x.Id));
